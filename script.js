...
// result 
function child(pgData) {
  document.querySelector('.page-data').innerHTML = '';
  var getData = parseInt(pgData.getAttribute('for'));
  var newData = pageData[getData - 1];
  var pageResult = document.createElement('div');
  pageResult.innerHTML =
    `<p for="${newData}">Id     : ${newData.id}</p>
     <p for="name">      Name   : ${newData.name}</p>
     <p for="email">     Email  : ${newData.email}</p>`
  document.querySelector('.page-data').append(pageResult).classList.add('.results');
}
// prev button
